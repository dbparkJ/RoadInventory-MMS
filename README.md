...
지도와 왼쪽 트랙 표시는 최대 12개 트랙까지 서로 다른 색을 공유하며 선택한 실행 범위는
지도에서 흰색 테두리로 강조됩니다. `←`/`A`, `→`/`D`로 이전·다음 프레임을 이동할 수
있고 파노라마 좌우 이동 영역을 클릭해도 같은 방식으로 이동합니다. 데이터 탐색기,
공간 뷰어, 작업 설정 상단의 새 창 아이콘을 누르면 해당 컴포넌트를 두 번째 모니터로
분리할 수 있으며 상태는 기본 창과 계속 동기화됩니다.
