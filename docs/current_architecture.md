...
2026-09-08 점진적 구조 개선의 현재 상태는 [refactor 체크포인트](refactor/CHECKPOINT.md)와
[실제 검증 기록](refactor/VALIDATION.md)에 별도로 기록한다. 새 명세의 P0(배포·검증 신뢰성)는
이 문서의 기존 P0(작업자 지주 기능)와 다른 작업 ID 체계다. 새 build provenance/CI/비교기와
오류 진단 변경의 계약은 해당 기록을 참조하며, 아래 계산·편집 경계는 보존한다.
